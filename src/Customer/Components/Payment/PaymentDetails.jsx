...
import { confirmOrder, getOrderById, orderHistory } from "../../../State/Order/Action";
...
const PaymentDetails = ()=>{
  ...
  const handleChange = (e) => {
    ...
    value = value.match(/.{1,4}/g)?.join(' ') || value; // Group in 4s
    setCardNumber(value);
  };
  ...
  const handleChangeDate = (e) => {
    ...
    if (value.length <= 4) {
      if (value.length >= 3) {
        value = value.replace(/^(\d{2})(\d{1,2})$/, '$1/$2'); // Add slash after MM
      }
      setExpiryDate(value);
    }
  };
...
<div className="grid sm:px-10 lg:grid-cols-2 lg:px-20 xl:px-32">
  <div className="px-4 pt-8">
    ...
    <div className="mt-8 space-y-3 rounded-lg border bg-white px-2 py-4 sm:px-6">
      {order.order?.orderItems.map((item)=><PaymentCard item={item}/> )
    }
    </div>
  ...
  <div className="mt-10 bg-gray-50 px-4 pt-8 lg:mt-0">
    ...
    <div className="">
      ...
      <div className="flex">
        <div className="relative w-7/12 flex-shrink-0">
          ...
          maxLength="19" id="card-no" name="card-no" className="w-full rounded-md border border-gray-200 px-2 py-3 pl-11 text-sm shadow-sm outline-none focus:z-10 focus:border-blue-500 focus:ring-blue-500" placeholder="xxxx-xxxx-xxxx-xxxx" />
          <div className="pointer-events-none absolute inset-y-0 left-0 inline-flex items-center px-3">
        ...
        maxLength="5"
        name="credit-expiry" className="w-full rounded-md border border-gray-200 px-2 py-3 text-sm shadow-sm outline-none focus:z-10 focus:border-blue-500 focus:ring-blue-500" placeholder="MM/YY" />
        <input type="text" value={cvc}
        onChange={handleChangeCVC}
        maxLength="3" name="credit-cvc" className="w-1/6 flex-shrink-0 rounded-md border border-gray-200 px-2 py-3 text-sm shadow-sm outline-none focus:z-10 focus:border-blue-500 focus:ring-blue-500" placeholder="CVC" />
      </div>
      ...
      <div className="mt-6 border-t border-b py-2">
        <div className="flex items-center justify-between">
          ...
          <p className="font-semibold text-gray-900">₹{order.order?.totalDiscountedPrice}</p>
        </div>
        <div className="flex items-center justify-between">
      ...
      <div className="mt-6 flex items-center justify-between">
        ...
        <p className="text-2xl font-semibold text-gray-900">₹{order.order?.totalDiscountedPrice}</p>
      </div>
    </div>
    <button onClick={handlePayment} className="mt-4 mb-8 w-full rounded-md bg-gray-900 px-6 py-3 font-medium text-white">Place Order</button>
       { 
       flag?<div class="px-8 py-6 bg-green-400 text-white flex justify-between rounded">
